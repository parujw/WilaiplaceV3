...
export default async function InvoicePrintPage({
  ...
  return (
    <main className={`${sarabun.className} sheet`}>
      <PrintButton backHref={`/bills/${encodeURIComponent(bill.id)}`} />

      {/*
        ห่อทั้งใบด้วยตารางเพื่อให้หัวกระดาษซ้ำทุกหน้า
        เบราว์เซอร์พิมพ์ thead ซ้ำเองทุกครั้งที่ตารางข้ามหน้า เป็นวิธีเดียวที่ได้ผลจริงทุกเครื่อง
        (position: fixed แต่ละเบราว์เซอร์ทำไม่เหมือนกัน ส่วน running header ของ @page ยังไม่มีใครรองรับ)
        บิลที่รายการยาวจนล้นไปหน้าสองจะได้ยังรู้ว่าเป็นใบเดียวกัน ไม่ใช่กระดาษหลงมา
      */}
      <table className="doc">
        <thead>
          <tr>
