...
| ใครมีลิงก์ก็เข้าได้ | Apps Script deploy แบบ "Anyone" | Firebase Auth + Firestore Rules (จำกัดรายชื่อเพิ่มได้ด้วย `LOGIN_ALLOWLIST_ONLY`) |
...
- Firebase Auth (Google Sign-in) — ค่าเริ่มต้นคือใครล็อกอินสำเร็จก็เข้าได้ในฐานะเจ้าของ
  ตั้ง `LOGIN_ALLOWLIST_ONLY=1` เมื่อไหร่จะยอมเฉพาะอีเมลใน `OWNER_EMAILS` หรือ `allowlist`
